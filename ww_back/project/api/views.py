...
@api_view(["GET"])
@csrf_exempt
def getManufacturerById(request,id):
    try:
        mfr = models.ManufacturerUser.objects.get(id=id)
    except models.ManufacturerUser.DoesNotExist as error:
        return Response({"error": str(error)}, status=status.HTTP_404_NOT_FOUND)

    if (request.method == "GET"):
        serialized = serialize.ManufacturerSerializer(mfr)
        return Response(serialized.data)
...
@csrf_exempt
def manageProfile(request, email):
    loggedUser = request.user
    try:
        desiredUser = models.User.objects.get(email=email)
    ...
    if (request.method == "GET"):
        ...
        if (email == loggedUser.email):
            if (loggedUser.is_customer):
                ...
                serialized = serialize.ManufacturerSerializer(instance=loggedUser.manufacturer, data=request.data,
                                                              partial=True)
                if (serialized.is_valid()):
                    serialized.save()
                    return Response(serialized.data)
...
@api_view(["GET"])
@csrf_exempt
def searchProducts(request, searchWord, cat, orderby, order):
    if cat != 9999:
        products = models.Product.objects.filter(name__icontains = searchWord,cat_id=cat).order_by(("-"*order)+str(orderby))
    else:
        products = models.Product.objects.filter(name__icontains = searchWord).order_by(("-" * order) + str(orderby))
...
@csrf_exempt
def getHistory(request,user_id): # request.user.id == user_id ?
    userHistory = models.History.objects.filter(user_id = user_id)
...
@permission_classes([IsAuthenticatedOrReadOnly])
@csrf_exempt
def getComments(request, prod_id):
    comments = models.Comment.objects.filter(prod_id = prod_id)
    if(request.method == "GET"):    
...
def rateProduct(request, prod_id):
    ...
    usersRating = int(data["rating"])

    newRating = round((((oldRating * product.ratings_num) + usersRating)/(product.ratings_num+1)), 2)
